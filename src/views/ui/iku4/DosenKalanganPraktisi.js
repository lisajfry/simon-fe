...
const DosenKalanganPraktisiList = () => {
    const [dosenKalanganPraktisiList, setDosenKalanganPraktisiList] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const { filteredIku4List, count } = useDosenKalanganPraktisi();
    ...
    useEffect(() => {
        ...
        const fetchDosenKalanganPraktisiList = async () => {
            try {
                const berkasMap = await fetchBerkasList();
                const dosenKalanganPraktisiListWithNama = await Promise.all(filteredIku4List.map(async (iku4) => {
                    let namaDosen;
                    if (iku4.NIDN) {
                ...
                setDosenKalanganPraktisiList(dosenKalanganPraktisiListWithNama);
                setLoading(false);
            } catch (error) {
        ...
        fetchDosenKalanganPraktisiList();
    }, [filteredIku4List]);
    ...
    const deleteIku4 = async (iku4_id) => {
        ...
        if (confirmDelete) {
            try {
                ...
                setDosenKalanganPraktisiList(dosenKalanganPraktisiList.filter(item => item.iku4_id !== iku4_id));
            } catch (error) {
        ...
        <Col>
            ...
            <Card>
                ...
                <div style={{ textAlign: 'center' }}>
                    <CardTitle>Tabel Dosen dari Kalangan Praktisi</CardTitle>
                </div>
                ...
                <CardBody>
                    <Table>
                        ...
                        <tbody>
                            {dosenKalanganPraktisiList.map((iku4, index) => (
                                <tr key={index}>
...
export default DosenKalanganPraktisiList;
